...
Validation is done during input to ensure, that all information is provided. After all data is entered, the deployment record is created and signed with the private key of the selected key pair. The deployment record is the send to the service to be stored in the blockchain.
...
- [ ] Add a service to receive the deployment records and store them in memory

...
- [x] Load the key pair from Windows-Keychain/DPAPI
- [ ] Move all code into a subfolger `cli`
- [ ] Add `pytest.ini`, `conftest.py`, `*.code-workspace`, `helpers.py` and the new description in `README.md` in my template
- [ ] Main Menu with key generation management as option
- [ ] Menu item to generate a new key pair and store it in Windows-Keychain/DPAPI
- [ ] Enter the name of the key
- [ ] Prevent entering already existing names
- [ ] Show the mnemonic seed phrase to the user and ask to write it down
- [ ] Store the key pair in Windows-Keychain/DPAPI or go back to the key pair management menu
